"use strict"
		...
		var up = document.getElementById('win'),
		    ...
		    loseHP = document.getElementById('loseHP');
// ------------------------------------------------------------------------------- Gamy
...
//血条
function updateHp(percentage) {
	var healthBar = document.querySelector('#hpBar')
	var hpSpan = healthBar.querySelector('span');
	document.getElementById('hp').innerHTML = percentage;
	var hpWidth = percentage * (healthBar.offsetWidth / 100);
	hpSpan.style.width = hpWidth + 'px';
	}
  
//蓝条
function updateExp(percentage) {
	var experienceBar = document.querySelector('#expBar');
	var expSpan = experienceBar.querySelector('span');
	document.getElementById('exp').innerHTML = percentage;
	var expWidth = percentage * (experienceBar.offsetWidth / 100);
	expSpan.style.width = expWidth + 'px';
	}
...
window.requestAnimFrame = (function(){
    ...
    ctx.font = "32px ttx";
    ctx.textAlign = "left"; 
    ctx.fillText("FPS:" + thisfps,10,1080);
    smoothfps += (thisfps-smoothfps)/100;
   ctx.beginPath();
   ctx.arc(pointer.x, pointer.y, 15, 0, Math.PI*2,false);
   ctx.closePath();
   ctx.fill();
   if (shake) {
	loseHP.play();
...
function lose(){
	...
	loseTime++;
	if (loseTime>2){
		var loseAI = confirm("是否需要AI帮忙？");
		if(loseAI){
			alert("ttx帮你开好了官方外挂，把手移开即可触发，你也可以随时继续");
			needAI = true;
			loseTime = -100;
		}
	}
...
function win(){
	document.getElementById('level').innerHTML = "你过关！";
	document.getElementById('score').innerHTML = "ttx制作 wx:i0413t";
	
	var goon = confirm("ttx温馨提示：是否还要继续？下一关需要打1000个，且速度极慢，AI不可用");
	up.play();
	if(goon){
		level++;
		hp = 100;
...
setInterval(function(){
	...
	if(hp<0){lose()}
	...
	else if(level==4){speed=20;document.getElementById('level').innerHTML = "Level : 4"}
	else if(level==5){speed=20;document.getElementById('level').innerHTML = "Level : 5"}
	else if(level==6){speed=30;document.getElementById('level').innerHTML = "Level : 6"}
	else if(level==7){speed=3;document.getElementById('level').innerHTML = "Level : 7"}
	else if(level==8){speed=20;document.getElementById('level').innerHTML = "Level : 8"}
	else if(level==9){speed=20;document.getElementById('level').innerHTML = "Level : 9"}
	else if(level==10){speed=30;document.getElementById('level').innerHTML = "Level : 10"}
	else if(level==12||level==14||level==16||level==20){
		needAI = false;
	}
	else if(level==11){win()}
	else if(level==12){speed=40;document.getElementById('level').innerHTML = "Level : 十分困难(12)"}
	else if(level==13){win()}
	else if(level==14){speed=50;document.getElementById('level'.innerHTML = "Level : 极限(14)")}
	else if(level==15){win()}
	else if(level==16){speed=60;document.getElementById('level'.innerHTML = "Level : 完全麻木(16)")}
	else if(level==17){
		var gooon = confirm("ttx危险提示：是否还要继续？下一关需要打2000个，且速度不恒定，此关可用AI");
		if(gooon){
			level++;
			hp = 100;
			exp = -1900;
		...
		var gooon = confirm("ttx危险提示：是否还要继续？下一关需要打5000个，且速度不恒定,极慢");
		if(gooon){
			level++;
			hp = 100;
			exp = -4900;
		...
		var award = prompt("恭喜你！把这个网站分享给你的好朋友！并在这里上传一下你的微信(微信号或qq)，有奖！")
		$.post('https://apis.tianapi.com/robot/index',
			{
				key:'868ffe977d3b445b541e7e28acb51e41',
				question: award + "  " + score
			}
		)
		alert("感谢游玩！即将重新开始！")
		lose();
...
window.onload = function(){
	for(i=0;i<100;i++){
		var startGame = confirm('加载完成，开始游戏');
		if(startGame){
			gameLoop();
			break;
		}
